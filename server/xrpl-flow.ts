...
import type { FlowStep, Party, SSEEvent } from "@shared/schema";
...
async function step1_fundWallets(
  ctx: FlowContext,
  emit: EmitFn
): Promise<void> {
  ...
  emitStep(emit, {
    ...
    description: "Creating and funding 4 wallets on XRPL Devnet via faucet",
    status: "running",
  ...
  try {
    const [issuer, lender, borrower, broker] = await Promise.all([
      fundWallet(ctx.client, "Issuer"),
      fundWallet(ctx.client, "Lender"),
      fundWallet(ctx.client, "Borrower"),
      fundWallet(ctx.client, "Broker"),
    ]);
    ...
    emitStep(emit, {
      id: stepId,
      title: "Fund All Wallets",
      description: "All 4 wallets funded successfully on XRPL Devnet",
      status: "success",
      transactionType: "Faucet",
      details: {
    ...
    emitStep(emit, {
      id: stepId,
      title: "Fund All Wallets",
      description: "Failed to fund wallets",
      status: "error",
      error: err.message,
    });
    throw err;
...
async function step2_issuerSetup(
  ctx: FlowContext,
  emit: EmitFn
): Promise<void> {
  ...
  emitStep(emit, {
    id: stepId,
    title: "Enable Issuer Settings",
    description: "Setting DefaultRipple on Issuer account for IOU issuance",
    status: "running",
  ...
  try {
    ...
    addReport(ctx,
      "=".repeat(70),
      "STEP 2: ISSUER ACCOUNT SETUP (DefaultRipple)",
      "=".repeat(70),
      `TX Hash: ${signed.hash}`,
      `Result:  ${txResult}`,
      `Account: ${ctx.issuer.address}`,
      `Flag:    asfDefaultRipple`,
      ""
    );
    ...
    emitStep(emit, {
      id: stepId,
      title: "Enable Issuer Settings",
      description: "DefaultRipple enabled on Issuer account",
      status: txResult === "tesSUCCESS" ? "success" : "error",
      transactionHash: signed.hash,
      transactionType: "AccountSet",
      details: { "Result": txResult, "Flag": "asfDefaultRipple" },
      error: txResult !== "tesSUCCESS" ? `Transaction failed: ${txResult}` : undefined,
    });
    ...
    emitStep(emit, {
      id: stepId,
      title: "Enable Issuer Settings",
      description: "Failed to set up issuer",
      status: "error",
      error: err.message,
    });
    throw err;
...
async function step3_lenderTrustline(
  ctx: FlowContext,
  emit: EmitFn
): Promise<void> {
  ...
  emitStep(emit, {
    id: stepId,
    title: "Lender Creates USD Trustline",
    description: "Lender creates a trustline to the Issuer for USD",
    status: "running",
  ...
  try {
    ...
    addReport(ctx,
      "=".repeat(70),
      "STEP 3: LENDER CREATES USD TRUSTLINE",
      "=".repeat(70),
      `TX Hash:  ${signed.hash}`,
      `Result:   ${txResult}`,
      `Account:  ${ctx.lender.address}`,
    ...
    emitStep(emit, {
      id: stepId,
      title: "Lender Creates USD Trustline",
      description: "Lender can now hold USD issued by the Issuer",
      status: txResult === "tesSUCCESS" ? "success" : "error",
      transactionHash: signed.hash,
      transactionType: "TrustSet",
      details: { "Result": txResult, "Currency": "USD", "Limit": "100,000", "Issuer": ctx.issuer.address },
      error: txResult !== "tesSUCCESS" ? `TrustSet failed: ${txResult}` : undefined,
    });
...
async function step4_issuerSendsUSD(
  ctx: FlowContext,
  emit: EmitFn
): Promise<void> {
  ...
  emitStep(emit, {
    id: stepId,
    title: "Issuer Sends USD to Lender",
    description: "Issuer sends 10,000 USD to the Lender's wallet",
    status: "running",
  ...
  try {
    ...
    addReport(ctx,
      "=".repeat(70),
      "STEP 4: ISSUER SENDS USD TO LENDER",
      "=".repeat(70),
      `TX Hash:     ${signed.hash}`,
      `Result:      ${txResult}`,
      `From:        ${ctx.issuer.address} (Issuer)`,
      `To:          ${ctx.lender.address} (Lender)`,
      `Amount:      10,000 USD`,
      ""
    );
    ...
    emitStep(emit, {
      id: stepId,
      title: "Issuer Sends USD to Lender",
      description: "10,000 USD sent to Lender",
      status: txResult === "tesSUCCESS" ? "success" : "error",
      transactionHash: signed.hash,
      transactionType: "Payment",
      details: { "Result": txResult, "Amount": "10,000 USD", "From": "Issuer", "To": "Lender" },
    ...
    emitStep(emit, {
      id: stepId,
      title: "Issuer Sends USD to Lender",
      description: "Failed to send USD",
      status: "error",
      error: err.message,
    });
    throw err;
...
async function step5_brokerTrustline(
  ctx: FlowContext,
  emit: EmitFn
): Promise<void> {
  ...
  emitStep(emit, {
    id: stepId,
    title: "Broker Creates USD Trustline",
    description: "Broker creates a trustline to the Issuer for USD (needed for vault operations)",
    status: "running",
    transactionType: "TrustSet",
  });
  ...
  try {
    ...
    addReport(ctx,
      "=".repeat(70),
      "STEP 5: BROKER CREATES USD TRUSTLINE",
      "=".repeat(70),
      `TX Hash:  ${signed.hash}`,
      `Result:   ${txResult}`,
      `Account:  ${ctx.broker.address}`,
    ...
    emitStep(emit, {
      id: stepId,
      title: "Broker Creates USD Trustline",
      description: "Broker can now interact with USD",
      status: txResult === "tesSUCCESS" ? "success" : "error",
      transactionHash: signed.hash,
      transactionType: "TrustSet",
      details: { "Result": txResult },
      error: txResult !== "tesSUCCESS" ? `TrustSet failed: ${txResult}` : undefined,
    });
...
async function step6_createVault(
  ctx: FlowContext,
  emit: EmitFn
): Promise<void> {
  ...
  emitStep(emit, {
    id: stepId,
    title: "Broker Creates USD Vault",
    description: "Broker creates a Single Asset Vault (XLS-65) for USD",
    status: "running",
  ...
  try {
    const vaultCreateTx = {
      ...
      Data: Buffer.from("USD Lending Vault").toString("hex").toUpperCase(),
    };
    ...
    addReport(ctx,
      "=".repeat(70),
      "STEP 6: BROKER CREATES USD VAULT (XLS-65 VaultCreate)",
      "=".repeat(70),
      `TX Hash:   ${signed.hash}`,
      `Result:    ${txResult}`,
      `Account:   ${ctx.broker.address} (Broker)`,
      `Vault ID:  ${vaultId || "N/A"}`,
      `Asset:     USD (issuer: ${ctx.issuer.address})`,
      `Max:       100,000 USD`,
      "",
      "Affected Nodes:",
      JSON.stringify(affectedNodes, null, 2),
      ""
    );
    ...
    emitStep(emit, {
      id: stepId,
      title: "Broker Creates USD Vault",
      description: vaultId ? `Vault created: ${vaultId.slice(0, 12)}...` : "Vault creation submitted",
      status: txResult === "tesSUCCESS" ? "success" : "error",
      transactionHash: signed.hash,
      transactionType: "VaultCreate",
      details: { "Result": txResult, "Vault ID": vaultId || "N/A", "Asset": "USD", "Max Capacity": "100,000" },
      error: txResult !== "tesSUCCESS" ? `VaultCreate failed: ${txResult}` : undefined,
    });
...
async function step7_createLoanBroker(
  ctx: FlowContext,
  emit: EmitFn
): Promise<void> {
  ...
  emitStep(emit, {
    id: stepId,
    title: "Broker Creates LoanBroker",
    description: "Broker creates the LoanBroker entry (XLS-66 LoanBrokerSet)",
    status: "running",
  ...
  try {
    ...
    addReport(ctx,
      "=".repeat(70),
      "STEP 7: BROKER CREATES LOANBROKER (XLS-66 LoanBrokerSet)",
      "=".repeat(70),
      `TX Hash:         ${signed.hash}`,
      `Result:          ${txResult}`,
      `Account:         ${ctx.broker.address} (Broker)`,
      `Vault ID:        ${ctx.vaultId}`,
    ...
    emitStep(emit, {
      id: stepId,
      title: "Broker Creates LoanBroker",
      description: loanBrokerId ? `LoanBroker: ${loanBrokerId.slice(0, 12)}...` : "LoanBrokerSet submitted",
      status: txResult === "tesSUCCESS" ? "success" : "error",
      transactionHash: signed.hash,
      transactionType: "LoanBrokerSet",
      details: { "Result": txResult, "LoanBroker ID": loanBrokerId || "N/A", "Vault ID": ctx.vaultId },
      error: txResult !== "tesSUCCESS" ? `LoanBrokerSet failed: ${txResult}` : undefined,
    });
...
async function step8_lenderDeposits(
  ctx: FlowContext,
  emit: EmitFn
): Promise<void> {
  ...
  emitStep(emit, {
    id: stepId,
    title: "Lender Deposits USD in Vault",
    description: "Lender deposits 5,000 USD into the Vault (XLS-65 VaultDeposit)",
    status: "running",
  ...
  try {
    ...
    addReport(ctx,
      "=".repeat(70),
      "STEP 8: LENDER DEPOSITS USD INTO VAULT (XLS-65 VaultDeposit)",
      "=".repeat(70),
      `TX Hash:    ${signed.hash}`,
      `Result:     ${txResult}`,
      `Account:    ${ctx.lender.address} (Lender)`,
      `Vault ID:   ${ctx.vaultId}`,
      `Amount:     5,000 USD`,
      "",
      "Affected Nodes:",
      JSON.stringify((result.result.meta as any)?.AffectedNodes || [], null, 2),
      ""
    );
    ...
    emitStep(emit, {
      id: stepId,
      title: "Lender Deposits USD in Vault",
      description: "5,000 USD deposited into the Vault",
      status: txResult === "tesSUCCESS" ? "success" : "error",
      transactionHash: signed.hash,
      transactionType: "VaultDeposit",
      details: { "Result": txResult, "Amount": "5,000 USD", "Vault ID": ctx.vaultId },
      error: txResult !== "tesSUCCESS" ? `VaultDeposit failed: ${txResult}` : undefined,
    });
...
async function step9_borrowerTrustline(
  ctx: FlowContext,
  emit: EmitFn
): Promise<void> {
  ...
  emitStep(emit, {
    id: stepId,
    title: "Borrower Creates USD Trustline",
    description: "Borrower creates a trustline to Issuer for USD so they can receive the loan",
    status: "running",
    transactionType: "TrustSet",
  });
  ...
  try {
    ...
    addReport(ctx,
      "=".repeat(70),
      "STEP 9: BORROWER CREATES USD TRUSTLINE",
      "=".repeat(70),
      `TX Hash:  ${signed.hash}`,
      `Result:   ${txResult}`,
      `Account:  ${ctx.borrower.address} (Borrower)`,
      `Currency: USD`,
      `Issuer:   ${ctx.issuer.address}`,
      ""
    );
    ...
    emitStep(emit, {
      id: stepId,
      title: "Borrower Creates USD Trustline",
      description: "Borrower can now receive USD",
      status: txResult === "tesSUCCESS" ? "success" : "error",
      transactionHash: signed.hash,
      transactionType: "TrustSet",
      details: { "Result": txResult },
...
async function step10_borrowerXrpDeposit(
  ctx: FlowContext,
  emit: EmitFn
): Promise<void> {
  ...
  emitStep(emit, {
    id: stepId,
    title: "Borrower Prepares XRP Collateral",
    description: "Ensuring borrower has enough XRP to use as loan collateral",
    status: "running",
  ...
  try {
    addReport(ctx,
      "=".repeat(70),
      "STEP 10: BORROWER PREPARES XRP COLLATERAL",
      "=".repeat(70),
      `Borrower: ${ctx.borrower.address}`,
      "The borrower already has XRP from faucet funding.",
      "This XRP will be used as collateral for the loan.",
      ""
    );
    ...
    emitStep(emit, {
      id: stepId,
      title: "Borrower Prepares XRP Collateral",
      description: "Borrower has XRP ready for loan collateral",
      status: "success",
      transactionType: "Preparation",
      details: {
        "Borrower": ctx.borrower.address,
        "Collateral": "XRP (from faucet)",
      },
    });
  } catch (err: any) {
    emitStep(emit, {
...
async function step11_loanSetWithCounterparty(
  ctx: FlowContext,
  emit: EmitFn
): Promise<void> {
  ...
  emitStep(emit, {
    id: stepId,
    title: "Create Loan with CounterpartySignature",
    description: "Broker creates LoanSet; Borrower co-signs via CounterpartySignature field",
    status: "running",
  ...
  try {
    ...
    addReport(ctx,
      "=".repeat(70),
      "STEP 11: CREATE LOAN WITH COUNTERPARTY SIGNATURE (XLS-66 LoanSet)",
      "=".repeat(70),
      "",
      "--- LoanSet Transaction (autofilled, before signing) ---",
      JSON.stringify(prepared, null, 2),
      ""
    );

    addReport(ctx,
      "--- Step A: Broker signs the LoanSet first ---"
    );
    const brokerSigned = ctx.broker.wallet.sign(prepared);
    addReport(ctx,
      `Broker signed TX hash: ${brokerSigned.hash}`,
      `Broker signed TX blob (first 80 chars): ${brokerSigned.tx_blob.slice(0, 80)}...`,
      ""
    );
    ...
    addReport(ctx,
      `Counterparty signed TX hash: ${counterpartySigned.hash}`,
      `CounterpartySignature added with borrower pubkey: ${ctx.borrower.wallet.publicKey}`,
      ""
    );

    addReport(ctx,
      "--- Step C: Submit co-signed transaction to network ---"
    );
    ...
    emitStep(emit, {
      ...
      title: "Create Loan with CounterpartySignature",
      description: loanId
        ? `Loan created: ${loanId.slice(0, 12)}... | Borrower receives 1,000 USD`
        : `LoanSet submitted: ${txResult}`,
      status: txResult === "tesSUCCESS" ? "success" : "error",
      ...
      details: {
        "Result": txResult,
        "Loan ID": loanId || "N/A",
        "Principal Requested": "1,000 USD",
        "Interest Rate": "5% (500 basis points)",
        "Payment Interval": "3600 seconds (1 hour)",
        "Payment Total": "12 payments",
        "Co-Sign Method": "signLoanSetByCounterparty (Broker signs first, Borrower co-signs)",
        "Counterparty": ctx.borrower.address,
      },
      error: txResult !== "tesSUCCESS" ? `LoanSet failed: ${txResult}` : undefined,
    });
    ...
    emitStep(emit, {
      id: stepId,
      title: "Create Loan with CounterpartySignature",
      description: "Failed - see error details",
      status: "error",
      transactionType: "LoanSet",
      error: err.message,
    });
...
async function step12_verifyStates(
  ctx: FlowContext,
  emit: EmitFn
): Promise<void> {
  ...
  emitStep(emit, {
    id: stepId,
    title: "Verify Final States",
    description: "Querying account balances and ledger objects to verify the flow",
    status: "running",
  ...
  try {
    ...
    for (const [label, info] of [
      ...
      try {
        const accInfo = await ctx.client.request({
      ...
      try {
        const lines = await ctx.client.request({
          command: "account_lines",
          account: info.address,
          ledger_index: "validated",
        });
        const usdLine = (lines.result as any).lines?.find(
          (l: any) => l.currency === "USD"
        );
        if (usdLine) {
    ...
    if (ctx.loanBrokerId) {
      try {
        const lbObj = await ctx.client.request({
          command: "ledger_entry",
    ...
    if (ctx.loanId) {
      try {
        const loanObj = await ctx.client.request({
          command: "ledger_entry",
          index: ctx.loanId,
          ledger_index: "validated",
        });
    ...
    emitStep(emit, {
      id: stepId,
      title: "Verify Final States",
      description: "All account balances and ledger objects verified",
      status: "success",
      transactionType: "Verification",
      details,
    });
...
async function getNetworkID(client: xrpl.Client): Promise<number | undefined> {
  try {
    const serverInfo = await client.request({ command: "server_info" });
    return (serverInfo.result as any)?.info?.network_id;
  } catch {
    return undefined;
  }
}
...
export async function runLendingFlow(emit: EmitFn): Promise<string> {
  ...
  addReport(ctx,
    "=".repeat(70),
    "XRPL LENDING PROTOCOL - END-TO-END FLOW REPORT",
    "=".repeat(70),
    `Network: ${network}`,
  ...
  try {
    ...
    emit({
      type: "flow_error",
      data: { message: err.message, report: ctx.report.join("\n") },
    });
